...
void ecrobot_device_initialize(void)
{
  ...
  Lab2a_circle_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(Lab2a_circle_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(Lab2a_circle_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Lab2a_circle_M, true);
    }
  }
...
TASK(OSEK_Task_main)
{
  ...
  while (1) {
    ...
    {                                  /* Sample time: [0.0s, 0.0s] */
      rtExtModeUpload(0, Lab2a_circle_M->Timing.t[0]);
    }

    {                                  /* Sample time: [0.005s, 0.0s] */
      rtExtModeUpload(1, ((Lab2a_circle_M->Timing.clockTick1) * 0.005));
    }
...
TASK(OSEK_Task_background)
{
  int status;
  int link_status = 0;
  while ((rtmGetErrorStatus(Lab2a_circle_M) == (NULL)) && !rtmGetStopRequested
         (Lab2a_circle_M)) {
    ...
    if (timesup == 0) {
      ...
      {
        boolean_T rtmStopReq = false;
        rtExtModePauseIfNeeded(Lab2a_circle_M->extModeInfo, 2, &rtmStopReq);
        if (rtmStopReq) {
          rtmSetStopRequested(Lab2a_circle_M, true);
        }

        if (rtmGetStopRequested(Lab2a_circle_M) == true) {
      ...
      {
        boolean_T rtmStopReq = false;
        rtExtModeOneStep(Lab2a_circle_M->extModeInfo, 2, &rtmStopReq);
        if (rtmStopReq) {
          rtmSetStopRequested(Lab2a_circle_M, true);
        }
      }
